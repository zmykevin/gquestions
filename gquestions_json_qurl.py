...
def crawlQuestions_Mingyang(start_paa, start_paa_url, paa_list, initialSet, query, browser, depth=0, lang="en"):
                        ...
                        if parent in currentQuestions:
                            ...
                            for l, value in new_q.items():
                                if value["q"].text == parent:
                                    continue
                                j['children'].append(
                                    {"name": value["q"].text, "parent": parent, "url": value["url"].get_attribute("href")})
...
def getCurrentSERP_Mingyang(browser):
    ...
    cnt = 0
    for q, url in zip(new_paa, new_paa_url):
        _tmpset.update({cnt: {"q": q, "url": url}})
        cnt += 1
